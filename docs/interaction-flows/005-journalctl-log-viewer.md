...
讓管理員在 Web UI 中直接查看任意服務的 systemd 日誌（`journalctl -u <service>`），不需 SSH 進機器。以側邊抽屜（Drawer）形式呈現，支援調整行數、WebSocket 即時串流（`journalctl -f`）與客戶端文字搜尋。
...
flowchart TD
    ...
    ShowLogs --> WsConnect[自動建立 WebSocket 連線<br>journalctl -f 持續串流]
    ShowEmpty --> WsConnect
    
    WsConnect --> WsStatus{WebSocket 狀態?}
    
    WsStatus -- 連線成功 LIVE --> Streaming[onMessage: 新行自動追加<br>自動捲動到底部<br>狀態指示器 ● LIVE]
    WsStatus -- 連線中斷 --> WsReconnect[自動重連<br>狀態指示器 ○ 離線]
    
    WsReconnect --> WsStatus
    Streaming --> UserInteract{管理員操作?}
    
    UserInteract -- 調整行數 --> ChangeLines[選擇 50/100/200/500<br>重新載入日誌<br>WebSocket 重新連線]
    UserInteract -- 搜尋關鍵字 --> SearchFilter[在日誌文字中<br>即時 highlight 匹配行]
    UserInteract -- 捲動瀏覽 --> Scroll[捲動查看<br>歷史日誌]
    UserInteract -- 點擊關閉 --> CloseDrawer[Drawer 滑出關閉<br>關閉 WebSocket 連線]
    UserInteract -- 手動重連 --> ManualReconnect[點擊重連按鈕<br>重新建立 WebSocket]
    
    ChangeLines --> FetchLogs
    ManualReconnect --> WsConnect
    ...
    style Streaming fill:#e8f5e9,stroke:#4caf50
    style WsReconnect fill:#fff3e0,stroke:#ff9800
```
...
flowchart TD
    ...
    Close1 --> Closed[Drawer 關閉<br>關閉 WebSocket 連線<br>回到 Dashboard]
    Close2 --> Closed
    Close3 --> Closed
...
| **系統回應** | 1. Loading spinner 消失<br>2. 日誌文字以等寬字體（monospace）顯示<br>3. 每行日誌完整呈現，支援水平捲動（`overflow-x: auto`）<br>4. 內容自動捲動到最底部（最新日誌）<br>5. Drawer 底部顯示控制列：行數選擇器 + 連線狀態指示器 |
...
### 步驟 4：即時串流（Live Streaming）

| | 描述 |
|---|------|
| **觸發** | Drawer 開啟且初始日誌載入完成後，前端自動建立 WebSocket 連線 |
| **操作前** | 日誌為靜態快照，連線狀態指示器尚未顯示 |
| **系統回應** | 1. 前端發起 WebSocket 連線至 `/api/v1/services/{name}/logs/stream`<br>2. 後端透過 WebSocket 執行 `journalctl -u {name} -f --no-pager -o short-iso` 持續串流<br>3. 每當有新日誌行，後端透過 WebSocket 推送至前端<br>4. 前端收到訊息後自動追加到日誌區塊底部<br>5. 若使用者在底部則自動捲動，若正在瀏覽歷史則不捲動<br>6. 連線狀態指示器顯示「● LIVE」 |
| **操作後** | 連線狀態指示器持續顯示「● LIVE」，日誌即時更新。管理員關閉 Drawer 時 WebSocket 自動關閉 |
| **狀態變化** | WebSocket：關閉 → 連線中 → 已連線（LIVE）<br>連線中斷時：LIVE → 自動重連中 → LIVE 或 ○ 離線 |
...
| **操作前** | Drawer 開啟中，WebSocket 可能正在串流 |
| **系統回應** | 1. 關閉 WebSocket 連線<br>2. Drawer 向右滑出（動畫約 200ms）<br>3. 遮罩淡出<br>4. Dashboard 恢復可互動狀態 |
...
| **WebSocket 連線中斷** | 不中斷已載入日誌顯示。連線狀態指示器變為「○ 離線」，前端自動嘗試重連（exponential backoff） | 若自動重連恢復，指示器回到「● LIVE」；若持續失敗則顯示手動重連按鈕 |
| **WebSocket 重連失敗** | 連線狀態指示器持續顯示「○ 離線」，控制列出現手動重連按鈕 | 管理員點擊手動重連按鈕重新建立 WebSocket |
...
| **WebSocket 重連策略** | 斷線後自動重連（exponential backoff，初始 1 秒，上限 30 秒） |
| **WebSocket 連線中斷處理** | 中斷後自動嘗試重連，若持續失敗則顯示手動重連按鈕 |
...
- [ ] 即時串流連線狀態指示器顯示 ● LIVE / ○ 離線
- [ ] WebSocket 連線建立後，journalctl -f 即時串流新行並自動捲動到底部
- [ ] 搜尋框輸入關鍵字後即時 highlight 匹配行，顯示匹配數量
- [ ] 關閉 Drawer 的四種方式皆有效（✕ / 遮罩 / Esc / 切換服務）
- [ ] 關閉 Drawer 時關閉 WebSocket 連線
...
- [ ] 測試無日誌服務的空狀態
- [ ] 測試 WebSocket 即時串流：服務產生新日誌時前端自動顯示
