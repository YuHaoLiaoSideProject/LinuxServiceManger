...
| **系統回應** | 在頁面內展開新增表單（或跳出 Modal）。表單包含：Channel 類型下拉選單、依類型動態顯示的 URL/Token 欄位、Channel 名稱輸入框、觸發事件 checkbox 群組、服務範圍 radio + 多選搜尋 |
...
| **操作後** | 已勾選的事件以藍色標示。下方服務範圍區塊：<br>• 「全部服務」radio（預設選中）— 任何服務狀態變更都觸發<br>• 「指定服務」radio + 多選搜尋下拉 — 僅選定的服務觸發。搜尋框支援文字過濾，從現有服務列表動態載入 |
