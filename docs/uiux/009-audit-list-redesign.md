...
### 決策 6：Mobile 分頁 — 3 鍵均分、44px、頁碼收合

- **≤767px**：`[‹ 上一頁] [第 1 / 7 頁] [下一頁 ›]` 三鍵均分全寬、**高 44px**（頁碼數字收合，避免 9 控件換行 3 行）
- **≥768px**：維持完整數字頁碼（含省略號），按鈕 min-height 36px
- `page-info`（「共 N 筆」）於 mobile 併入中央按鈕文字；桌機維持右側獨立 meta
...
| **分頁（≥768px）** | 數字頁碼＋省略號；active accent 底 | 點擊跳頁；disabled 淡化 |
| **分頁（≤767px）** | 三鍵均分 44px，中央顯示「第 X / Y 頁」 | 上一頁／下一頁；中央無動作 |
...
| 斷點 | 表格 | 分頁 | 觸控目標 |
|------|------|------|:---:|
| **≥1024px** | 7 欄完整表格；detail clamp 2 | 數字頁碼＋省略號，36px | 36px |
| **768–1023px** | 表格 `overflow-x:auto`（現況保留）；clamp 2 | 數字頁碼，36px | 36px |
| **≤767px** | 卡片 4 區塊重排（時間＋徽章標頭、動作次標題、meta、目標、detail） | 三鍵均分全寬 44px；中央「第 X / Y 頁」 | **44px** |
...
| **2.5.5 觸控** | 觸控目標 ≥44×44px | mobile 分頁按鈕 44px；Toolbar 已 44px |
...
| `.mobile-card-head` / `.mobile-card-meta` | mobile 卡片區塊 |
| `.pager-mobile` | ≤767px 三鍵均分（`grid-template-columns: 1fr 1fr 1fr`）、44px |
...
### 實作後續（不在本次範圍）
- [ ] `AuditTable.vue`：徽章／邊條／clamp／icon／key／aria 修正
- [ ] `useI18n.ts`：snake_case keys＋補齊 notify_*（zh-TW + en）
- [ ] `AuditLogView.vue`：分頁 mobile 三鍵、Empty/Error SVG
- [ ] `main.css`：mobile 卡片重排、分頁 44px
- [ ] 補 E2E：失敗列視覺驗證（現況 312 筆 0 失敗，失敗態從未測試）

---

*產出日期：2026-08-14 · 實測 base：commit `6e51188`*
