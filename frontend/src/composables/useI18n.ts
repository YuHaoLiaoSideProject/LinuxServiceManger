import { ref, readonly } from 'vue'

type Lang = 'zh-TW' | 'en'

const translations: Record<Lang, Record<string, string>> = {
  'zh-TW': {
    'stats.total': '總服務數',
    'stats.running': '執行中',
    'stats.failed': '失敗',
    'filter.all': '全部',
    'filter.running': '執行中',
    'filter.failed': '失敗',
    'filter.inactive': '未啟用',
    'stats.groupAria': '狀態過濾',
    'search.placeholder': '搜尋服務名稱...',
    'header.refresh': '重新整理',
    'header.logout': '🚪 登出',
    'tab.my': '我的服務',
    'tab.system': '系統服務',
    'modal.title': '⚠️ 確認操作',
    'modal.cancel': '取消',
    'modal.confirm': '確認',
    'modal.stop': '確定要停止 {name} 嗎？',
    'modal.restart': '確定要重啟 {name} 嗎？',
    'empty.state': '找不到任何服務，或無法連線至 systemd。',
    'search.empty': '沒有符合「{term}」的服務',
    'action.start': '啟動',
    'action.stop': '停止',
    'action.restart': '重啟',
    'action.logs': '日誌',
    'locked.badge': '🔒 已鎖定',
    'locked.tooltip': '此服務受保護，無法直接操作。設定 UNLOCKED_SERVICES 環境變數解鎖。',
    'toast.started': '{name} 已啟動',
    'toast.stopped': '{name} 已停止',
    'toast.restarted': '{name} 已重啟',
    'toast.enabled': '{name} 已設為開機自動啟動',
    'toast.disabled': '{name} 已取消開機自動啟動',
    'toast.error': '{name} 操作失敗',
    'modal.disable': '確定要停用 {name} 的開機自動啟動嗎？此服務下次重開機後將不會自動啟動。',
    'login.title': 'Linux Service Manager',
    'login.subtitle': '請登入以管理系統服務',
    'login.username': '帳號',
    'login.password': '密碼',
    'login.submit': '登入',
    'login.error': '帳號或密碼錯誤',
    'col.name': '名稱',
    'col.load': '載入狀態',
    'col.active': '啟用狀態',
    'col.sub': '執行狀態',
    'col.autoStart': '開機啟動',
    'col.actions': '操作',
    'caption.title': '系統服務列表',
    'caption.sub': '— 點擊操作按鈕管理服務',
    'action.start.aria': '啟動 {name}',
    'action.stop.aria': '停止 {name}',
    'action.restart.aria': '重啟 {name}',
    'action.logs.aria': '查看 {name} 日誌',
    'header.refresh.aria': '重新整理',
    'header.logout.aria': '登出',
    'lang.toggle.title': '切換語言',
    'theme.toggle': '切換主題',
    'theme.toggle.title': '切換深色/淺色主題',
    'nav.dashboard': '儀表板',
    'nav.audit': '稽核紀錄',
    'nav.tokens': 'API Tokens',
    'nav.notifications': '通知',
    'nav.nodes': 'Node Management',
    'nodes.total': '總節點數',
    'nodes.online': '線上台數',
    'nodes.offline': '離線台數',
    'nodes.totalServices': '總服務數',
    'nodes.activeServices': '執行中',
    'nodes.failedServices': '失敗',
    'nodes.addNode': '新增節點',
    'nodes.downloadAgent': '下載 Agent',
    'nodes.searchPlaceholder': '搜尋所有節點的服務…',
    'nodes.searchEmpty': '沒有找到匹配的服務',
    'nodes.failedNodes': '個節點無法查詢（離線/逾時）',
    'nodes.colName': '名稱',
    'nodes.colAddress': '位址',
    'nodes.colStatus': '狀態',
    'nodes.colHeartbeat': '最後心跳',
    'nodes.colVersion': '版本',
    'nodes.colActions': '操作',
    'nodes.colNotes': '備註',
    'nodes.deleteTitle': '移除節點',
    'nodes.deleteConfirm': '確定要移除此節點？',
    'nodes.deleteDesc': '所有歷史資料將保留。此操作無法復原。',
    'nodes.deleteConfirmBtn': '確認移除',
    'nodes.noNodes': '尚無已註冊節點',
    'nodes.noMatch': '找不到符合條件的節點',
    'nodes.emptyDesc': '請先在目標 Linux 機器上部署 Agent，然後點擊「新增節點」進行註冊。',
    'nodes.loading': '載入中...',
    'nodes.loadError': '載入節點失敗',
    'nodes.removed': '已移除「{name}」',
    'nodes.removeError': '移除失敗',
    'nodes.tableLabel': '節點列表',
    'nodes.offlineTooltip': '節點離線',
    'nodes.longOfflineTooltip': '節點長期離線',
    'nodes.warningTooltip': '心跳延遲',
    'nodes.heartbeat': '心跳：{time}',
    'nodes.edit': '編輯',
    'nodes.remove': '移除',
    'nodes.close': '關閉',
    'nodes.cancel': '取消',
    'nodes.searchLabel': '搜尋節點',
    'nodes.searchNodePlaceholder': '搜尋節點名稱、位址…',
    'nodes.clearSearch': '清除搜尋',
    'status.online': '線上',
    'status.warning': '延遲',
    'status.offline': '離線',
    'status.long_offline': '長期離線',
    'time.secondsAgo': '{sec} 秒前',
    'time.minutesAgo': '{min} 分鐘前',
    'time.hoursAgo': '{hr} 小時前',
    'time.daysAgo': '{day} 天前',
    'account.signedIn': '已登入',
    'account.toggle.title': '帳號選單',
    'menu.sectionLinks': '功能',
    'menu.notifications': '🔔 通知',
    'menu.docs': '📖 API 文件',
    'menu.sectionSettings': '設定',
    'menu.apiTokens': 'API Tokens',
    'menu.toggleTheme': '☀️ 切換主題',
    'menu.toggleLang': '🌐 切換語言',
    'menu.logout': '🚪 登出',
    'menu.logout.aria': '登出',
    'search.aria': '搜尋服務',
    'search.clear.aria': '清除搜尋內容',
    'search.clear.title': '清除搜尋',
    'search.regex.title': '切換正則搜尋',
    'filter.count.shown': '顯示',
    'filter.count.total': '/ 共',
    'batch.selected.prefix': '已選取',
    'batch.selected.suffix': '個服務',
    'batch.hint': '勾選服務後，可在此批次 啟動 / 停止 / 重啟',
    'batch.progress': '{done}/{total} 完成',
    'batch.progress.aria': '批次執行進度',
    'batch.clear': '取消選取',
    'batch.action.start.aria': '啟動 {count} 個服務',
    'batch.action.stop.aria': '停止 {count} 個服務',
    'batch.action.restart.aria': '重啟 {count} 個服務',
    'status.load.loaded': '已載入',
    'status.load.not-found': '未找到',
    'status.load.bad-setting': '設定錯誤',
    'status.load.error': '錯誤',
    'status.load.masked': '已遮蔽',
    'status.load.stub': '存根',
    'status.active.active': '啟用中',
    'status.active.inactive': '未啟用',
    'status.active.failed': '失敗',
    'status.active.activating': '啟用中',
    'status.active.deactivating': '停用中',
    'status.active.reloading': '重載中',
    'status.sub.running': '執行中',
    'status.sub.dead': '已停止',
    'status.sub.exited': '已退出',
    'status.sub.failed': '失敗',
    'status.sub.auto-restart': '自動重啟',
    'status.sub.plugged': '已插入',
    'status.sub.mounted': '已掛載',
    'status.sub.waiting': '等待中',
    'status.sub.listening': '監聽中',
    'autoStart.na': '不適用',
    'autoStart.on': 'ON',
    'autoStart.off': 'OFF',
    'autoStart.enableAria': '開啟 {name} 的自動啟動',
    'autoStart.disableAria': '關閉 {name} 的自動啟動',
    'audit.title': '稽核紀錄',
    'audit.col.time': '時間',
    'audit.col.user': '使用者',
    'audit.col.sourceIp': '來源 IP',
    'audit.col.action': '動作',
    'audit.col.target': '目標服務',
    'audit.col.result': '結果',
    'audit.col.detail': '詳細資訊',
    'audit.action.login': '登入',
    'audit.action.logout': '登出',
    'audit.action.start': '啟動',
    'audit.action.stop': '停止',
    'audit.action.restart': '重啟',
    'audit.action.enable': '啟用',
    'audit.action.disable': '停用',
    'audit.action.config_view': '檢視設定檔',
    'audit.action.config_save': '儲存設定檔',
    'audit.action.token_create': '建立 Token',
    'audit.action.token_revoke': '撤銷 Token',
    'audit.action.notify_create': '建立通知 Channel',
    'audit.action.notify_update': '更新通知 Channel',
    'audit.action.notify_delete': '刪除通知 Channel',
    'audit.action.notify_toggle': '切換通知 Channel',
    'audit.action.notify_test': '測試通知 Channel',
    'audit.result.success': '成功',
    'audit.result.failure': '失敗',
    'audit.searchPlaceholder': '搜尋操作者、動作、目標服務...',
    'audit.matched.prefix': '符合',
    'audit.matched.suffix': '筆記錄',
    'audit.exportCsv': '匯出 CSV',
    'audit.exportSuccess': '稽核紀錄已匯出',
    'audit.exportFailed': '匯出失敗',
    'audit.pagination.prev': '上一頁',
    'audit.pagination.next': '下一頁',
    'audit.pagination.info': '第 {page} / {total} 頁，共 {count} 筆',
    'audit.pagination.pageOf': '第 {page} / {total} 頁',
    'audit.noRecords': '尚無操作紀錄',
    'audit.noMatch': '沒有符合條件的紀錄',
    'audit.clearFilters': '清除條件',
    'audit.retry': '重試',
    'audit.dateFrom': '開始日期',
    'audit.dateTo': '結束日期',
    'audit.dateError': '開始日期不能晚於結束日期',
    'action.config.edit': 'Edit Config',
    'action.config.view': 'View Config',
    'action.config.edit.aria': '編輯 {name} 設定檔',
    'action.config.view.aria': '檢視 {name} 設定檔',
    'config.loading': '載入設定檔中...',
    'config.validate': 'Validate',
    'config.save': 'Save',
    'config.cancel': 'Cancel',
    'config.close': 'Close',
    'config.closeAria': '關閉',
    'config.saveTitle': '儲存設定檔變更',
    'config.saveChanges': 'Save Changes',
    'config.saveConfirm': '確定要將變更寫入 {path} 嗎？',
    'config.saveReloadNotice': '儲存後將自動執行 systemctl daemon-reload 使變更生效',
    'config.saveRisk': '⚠️ 錯誤的設定可能導致服務無法啟動。',
    'config.saveEmptyWarning': '⚠️ 設定檔內容為空。儲存空設定檔可能導致 systemd 無法解析。確定要繼續嗎？',
    'config.validatePass': '語法驗證通過 — 設定檔語法正確',
    'config.validateUnavailable': '無法執行語法驗證 — systemd-analyze 不可用或執行錯誤。您仍可直接儲存設定檔。',
    'config.validateEmpty': '設定檔內容為空，請先編輯或載入內容',
    'config.validateRequestError': '請求格式錯誤',
    'config.notFound': '設定檔不存在：{path}。請確認服務設定檔是否已被手動刪除。',
    'config.largeFile': '設定檔較大（{size}），編輯時可能有效能影響。',
    'config.discardTitle': '有未儲存的變更',
    'config.discardMessage': '有未儲存的變更，確定要離開嗎？未儲存的變更將會遺失。',
    'config.stay': 'Stay',
    'config.discardChanges': 'Discard Changes',
    'config.saved': '{name} 設定檔已儲存，daemon-reload 已執行',
    'config.saveFailed': '儲存失敗：{error}',
    'config.reloadFailed': '設定檔已儲存，但 daemon-reload 失敗：{error}。請手動執行 systemctl daemon-reload。備份檔：{backupPath}',
    'config.conflict': '設定檔已被其他使用者修改。請重新載入後再編輯。',
    'config.conflictTitle': '設定檔衝突',
    'config.conflictReloaded': '已重新載入最新設定檔',
    'config.reload': '重新載入',
    'config.discarded': '已放棄未儲存的變更',
    'config.back': '返回',
    'config.retry': '重試',
    'notify.addChannel': '新增 Channel',
    'notify.tabChannels': 'Channel 設定',
    'notify.tabHistory': '發送紀錄',
    'notify.channelType': 'Channel 類型',
    'notify.triggerEvents': '觸發事件',
    'notify.serviceScope': '服務範圍',
    'notify.allServices': '全部服務',
    'notify.specificServices': '指定服務',
    'notify.searchServices': '搜尋服務...',
    'notify.test': '測試',
    'notify.addHeader': '新增 Header（最多 10 組）',
    'notify.allChannels': '全部 Channel',
    'notify.resultAll': '全部',
    'notify.resultSuccess': '成功',
    'notify.resultFailure': '失敗',
    'notify.colTime': '時間',
    'notify.colChannel': 'Channel',
    'notify.colEvent': '觸發事件',
    'notify.colService': '目標服務',
    'notify.colResult': '發送結果',
    'notify.colError': '錯誤訊息',
    'notify.pageInfo': '第 {page} / {total} 頁',
  },
  'en': {
    'stats.total': 'Total Services',
    'stats.running': 'Running',
    'stats.failed': 'Failed',
    'filter.all': 'All',
    'filter.running': 'Running',
    'filter.failed': 'Failed',
    'filter.inactive': 'Inactive',
    'stats.groupAria': 'Filter by status',
    'search.placeholder': 'Search services...',
    'header.refresh': 'Refresh',
    'header.logout': '🚪 Logout',
    'tab.my': 'My Services',
    'tab.system': 'System Services',
    'modal.title': '⚠️ Confirm Action',
    'modal.cancel': 'Cancel',
    'modal.confirm': 'Confirm',
    'modal.stop': 'Are you sure you want to stop {name}?',
    'modal.restart': 'Are you sure you want to restart {name}?',
    'empty.state': 'No services found or unable to connect to systemd.',
    'search.empty': 'No services matching "{term}"',
    'action.start': 'Start',
    'action.stop': 'Stop',
    'action.restart': 'Restart',
    'action.logs': 'Logs',
    'locked.badge': '🔒 Locked',
    'locked.tooltip': 'This service is protected. Set the UNLOCKED_SERVICES environment variable to unlock.',
    'toast.started': '{name} started',
    'toast.stopped': '{name} stopped',
    'toast.restarted': '{name} restarted',
    'toast.enabled': '{name} enabled for auto-start',
    'toast.disabled': '{name} disabled for auto-start',
    'toast.error': '{name} operation failed',
    'modal.disable': 'Are you sure you want to disable auto-start for {name}? This service will not start automatically on next boot.',
    'login.title': 'Linux Service Manager',
    'login.subtitle': 'Sign in to manage system services',
    'login.username': 'Username',
    'login.password': 'Password',
    'login.submit': 'Sign In',
    'login.error': 'Invalid username or password',
    'col.name': 'Name',
    'col.load': 'Load',
    'col.active': 'Active',
    'col.sub': 'Sub',
    'col.autoStart': 'Auto-start',
    'col.actions': 'Actions',
    'caption.title': 'Service List',
    'caption.sub': '— Click action buttons to manage services',
    'action.start.aria': 'Start {name}',
    'action.stop.aria': 'Stop {name}',
    'action.restart.aria': 'Restart {name}',
    'action.logs.aria': 'View logs for {name}',
    'header.refresh.aria': 'Refresh',
    'header.logout.aria': 'Logout',
    'lang.toggle.title': 'Switch language',
    'theme.toggle': 'Toggle theme',
    'theme.toggle.title': 'Toggle dark/light theme',
    'nav.dashboard': 'Dashboard',
    'nav.audit': 'Audit Log',
    'nav.tokens': 'API Tokens',
    'nav.notifications': 'Notifications',
    'nav.nodes': 'Node Management',
    'nodes.total': 'Total Nodes',
    'nodes.online': 'Online',
    'nodes.offline': 'Offline',
    'nodes.totalServices': 'Total Services',
    'nodes.activeServices': 'Running',
    'nodes.failedServices': 'Failed',
    'nodes.addNode': 'Add Node',
    'nodes.downloadAgent': 'Download Agent',
    'nodes.searchPlaceholder': 'Search services on all nodes…',
    'nodes.searchEmpty': 'No matching services found',
    'nodes.failedNodes': 'node(s) unavailable (offline/timeout)',
    'nodes.colName': 'Name',
    'nodes.colAddress': 'Address',
    'nodes.colStatus': 'Status',
    'nodes.colHeartbeat': 'Last Heartbeat',
    'nodes.colVersion': 'Version',
    'nodes.colActions': 'Actions',
    'nodes.colNotes': 'Notes',
    'nodes.deleteTitle': 'Remove Node',
    'nodes.deleteConfirm': 'Are you sure you want to remove this node?',
    'nodes.deleteDesc': 'All history data will be preserved. This action cannot be undone.',
    'nodes.deleteConfirmBtn': 'Confirm Remove',
    'nodes.noNodes': 'No registered nodes yet',
    'nodes.noMatch': 'No matching nodes found',
    'nodes.emptyDesc': 'Please deploy the Agent on the target Linux machine first, then click "Add Node" to register.',
    'nodes.loading': 'Loading...',
    'nodes.loadError': 'Failed to load nodes',
    'nodes.removed': 'Removed "{name}"',
    'nodes.removeError': 'Remove failed',
    'nodes.tableLabel': 'Nodes list',
    'nodes.offlineTooltip': 'Node offline',
    'nodes.longOfflineTooltip': 'Node long-term offline',
    'nodes.warningTooltip': 'Heartbeat delayed',
    'nodes.heartbeat': 'Heartbeat: {time}',
    'nodes.edit': 'Edit',
    'nodes.remove': 'Remove',
    'nodes.close': 'Close',
    'nodes.cancel': 'Cancel',
    'nodes.searchLabel': 'Search nodes',
    'nodes.searchNodePlaceholder': 'Search node name, address…',
    'nodes.clearSearch': 'Clear search',
    'status.online': 'Online',
    'status.warning': 'Warning',
    'status.offline': 'Offline',
    'status.long_offline': 'Long Offline',
    'time.secondsAgo': '{sec} sec ago',
    'time.minutesAgo': '{min} min ago',
    'time.hoursAgo': '{hr} hr ago',
    'time.daysAgo': '{day} days ago',
    'account.signedIn': 'Signed in',
    'account.toggle.title': 'Account menu',
    'menu.sectionLinks': 'Links',
    'menu.notifications': '🔔 Notifications',
    'menu.docs': '📖 API Docs',
    'menu.sectionSettings': 'Settings',
    'menu.apiTokens': 'API Tokens',
    'menu.toggleTheme': '☀️ Toggle theme',
    'menu.toggleLang': '🌐 Switch language',
    'menu.logout': '🚪 Logout',
    'menu.logout.aria': 'Logout',
    'search.aria': 'Search services',
    'search.clear.aria': 'Clear search',
    'search.clear.title': 'Clear search',
    'search.regex.title': 'Toggle regex search',
    'filter.count.shown': 'Showing',
    'filter.count.total': 'of',
    'batch.selected.prefix': 'Selected',
    'batch.selected.suffix': 'services',
    'batch.hint': 'Select services to batch start / stop / restart',
    'batch.progress': '{done}/{total} done',
    'batch.progress.aria': 'Batch execution progress',
    'batch.clear': 'Clear selection',
    'batch.action.start.aria': 'Start {count} services',
    'batch.action.stop.aria': 'Stop {count} services',
    'batch.action.restart.aria': 'Restart {count} services',
    'status.load.loaded': 'Loaded',
    'status.load.not-found': 'Not Found',
    'status.load.bad-setting': 'Bad Setting',
    'status.load.error': 'Error',
    'status.load.masked': 'Masked',
    'status.load.stub': 'Stub',
    'status.active.active': 'Active',
    'status.active.inactive': 'Inactive',
    'status.active.failed': 'Failed',
    'status.active.activating': 'Activating',
    'status.active.deactivating': 'Deactivating',
    'status.active.reloading': 'Reloading',
    'status.sub.running': 'Running',
    'status.sub.dead': 'Dead',
    'status.sub.exited': 'Exited',
    'status.sub.failed': 'Failed',
    'status.sub.auto-restart': 'Auto-restart',
    'status.sub.plugged': 'Plugged',
    'status.sub.mounted': 'Mounted',
    'status.sub.waiting': 'Waiting',
    'status.sub.listening': 'Listening',
    'autoStart.na': 'N/A',
    'autoStart.on': 'ON',
    'autoStart.off': 'OFF',
    'autoStart.enableAria': 'Enable auto-start for {name}',
    'autoStart.disableAria': 'Disable auto-start for {name}',
    'audit.title': 'Audit Log',
    'audit.col.time': 'Time',
    'audit.col.user': 'User',
    'audit.col.sourceIp': 'Source IP',
    'audit.col.action': 'Action',
    'audit.col.target': 'Target',
    'audit.col.result': 'Result',
    'audit.col.detail': 'Detail',
    'audit.action.login': 'Login',
    'audit.action.logout': 'Logout',
    'audit.action.start': 'Start',
    'audit.action.stop': 'Stop',
    'audit.action.restart': 'Restart',
    'audit.action.enable': 'Enable',
    'audit.action.disable': 'Disable',
    'audit.action.config_view': 'View config',
    'audit.action.config_save': 'Save config',
    'audit.action.token_create': 'Create token',
    'audit.action.token_revoke': 'Revoke token',
    'audit.action.notify_create': 'Create notification channel',
    'audit.action.notify_update': 'Update notification channel',
    'audit.action.notify_delete': 'Delete notification channel',
    'audit.action.notify_toggle': 'Toggle notification channel',
    'audit.action.notify_test': 'Test notification channel',
    'audit.result.success': 'Success',
    'audit.result.failure': 'Failure',
    'audit.searchPlaceholder': 'Search user, action, target...',
    'audit.matched.prefix': 'Matched',
    'audit.matched.suffix': 'records',
    'audit.exportCsv': 'Export CSV',
    'audit.exportSuccess': 'Audit log exported',
    'audit.exportFailed': 'Export failed',
    'audit.pagination.prev': 'Previous',
    'audit.pagination.next': 'Next',
    'audit.pagination.info': 'Page {page} / {total}, {count} records',
    'audit.pagination.pageOf': 'Page {page} / {total}',
    'audit.noRecords': 'No audit records',
    'audit.noMatch': 'No matching records',
    'audit.clearFilters': 'Clear filters',
    'audit.retry': 'Retry',
    'audit.dateFrom': 'From date',
    'audit.dateTo': 'To date',
    'audit.dateError': 'Start date cannot be after end date',
    'action.config.edit': 'Edit Config',
    'action.config.view': 'View Config',
    'action.config.edit.aria': 'Edit {name} config',
    'action.config.view.aria': 'View {name} config',
    'config.loading': 'Loading config file...',
    'config.validate': 'Validate',
    'config.save': 'Save',
    'config.cancel': 'Cancel',
    'config.close': 'Close',
    'config.closeAria': 'Close',
    'config.saveTitle': 'Save Config Changes',
    'config.saveChanges': 'Save Changes',
    'config.saveConfirm': 'Write changes to {path}?',
    'config.saveReloadNotice': 'A systemctl daemon-reload will run automatically',
    'config.saveRisk': '⚠️ Incorrect settings may prevent the service from starting.',
    'config.saveEmptyWarning': '⚠️ Config content is empty. Saving an empty config may break systemd. Continue?',
    'config.validatePass': 'Syntax validation passed — config is valid',
    'config.validateUnavailable': 'Unable to validate — systemd-analyze unavailable or failed. You can still save.',
    'config.validateEmpty': 'Config content is empty. Edit or load content first.',
    'config.validateRequestError': 'Invalid request format',
    'config.notFound': 'Config file not found: {path}. It may have been deleted manually.',
    'config.largeFile': 'Large config file ({size}) — editing may be slow.',
    'config.discardTitle': 'Unsaved changes',
    'config.discardMessage': 'You have unsaved changes. Leave? Unsaved changes will be lost.',
    'config.stay': 'Stay',
    'config.discardChanges': 'Discard Changes',
    'config.saved': '{name} config saved, daemon-reload executed',
    'config.saveFailed': 'Save failed: {error}',
    'config.reloadFailed': 'Config saved, but daemon-reload failed: {error}. Run systemctl daemon-reload manually. Backup: {backupPath}',
    'config.conflict': 'Config modified by another user. Reload and edit again.',
    'config.conflictTitle': 'Config conflict',
    'config.conflictReloaded': 'Latest config reloaded',
    'config.reload': 'Reload',
    'config.discarded': 'Unsaved changes discarded',
    'config.back': 'Back',
    'config.retry': 'Retry',
    'notify.addChannel': 'Add Channel',
    'notify.tabChannels': 'Channels',
    'notify.tabHistory': 'History',
    'notify.channelType': 'Channel Type',
    'notify.triggerEvents': 'Trigger Events',
    'notify.serviceScope': 'Service Scope',
    'notify.allServices': 'All Services',
    'notify.specificServices': 'Specific Services',
    'notify.searchServices': 'Search services...',
    'notify.test': 'Test',
    'notify.addHeader': 'Add Header (max 10)',
    'notify.allChannels': 'All Channels',
    'notify.resultAll': 'All',
    'notify.resultSuccess': 'Success',
    'notify.resultFailure': 'Failure',
    'notify.colTime': 'Time',
    'notify.colChannel': 'Channel',
    'notify.colEvent': 'Event',
    'notify.colService': 'Service',
    'notify.colResult': 'Result',
    'notify.colError': 'Error',
    'notify.pageInfo': 'Page {page} / {total}',
  },
}

function detectLang(): Lang {
  const stored = localStorage.getItem('lms-lang') as Lang | null
  if (stored && translations[stored]) return stored
  if (navigator.language.startsWith('zh')) return 'zh-TW'
  return 'en'
}

const currentLang = ref<Lang>(detectLang())

export function useI18n() {
  function t(key: string, params?: Record<string, string>): string {
    const text = translations[currentLang.value]?.[key] || key
    if (!params) return text
    return Object.entries(params).reduce((s, [k, v]) => s.replace(`{${k}}`, v), text)
  }

  function setLang(lang: Lang) {
    currentLang.value = lang
    localStorage.setItem('lms-lang', lang)
    document.documentElement.lang = lang
  }

  function toggleLang() {
    setLang(currentLang.value === 'zh-TW' ? 'en' : 'zh-TW')
  }

  return { t, lang: readonly(currentLang), setLang, toggleLang }
}
